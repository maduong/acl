...
use Edutalk\Base\ACL\Http\DataTables\RolesListDataTable;
use Edutalk\Base\ACL\Http\Requests\CreateRoleRequest;
use Edutalk\Base\ACL\Http\Requests\UpdateRoleRequest;
...
class RoleController extends BaseAdminController
{
    ...
    /**
     * @var \Edutalk\Base\ACL\Repositories\RoleRepository
     */
    ...
    public function __construct(RoleRepositoryContract $roleRepository)
    {
        ...
        $this->middleware(function (Request $request, $next) {
            ...
            $this->breadcrumbs
                ...
                ->addLink(trans('edutalk-acl::base.roles'), route('admin::acl-roles.index.get'));
    ...
    {
        $this->setPageTitle(trans('edutalk-acl::base.roles'));

        $this->dis['dataTable'] = $rolesListDataTable->run();

        return do_filter(BASE_FILTER_CONTROLLER, $this, Edutalk_ACL_ROLE, 'index.get', $rolesListDataTable)->viewAdmin('roles.index');
    }
    ...
    public function postListing(RolesListDataTable $rolesListDataTable)
    {
        ...
        return do_filter(BASE_FILTER_CONTROLLER, $data, Edutalk_ACL_ROLE, 'index.post', $this);
    }
    ...
    protected function groupAction()
    {
        ...
        if ($this->request->get('customActionType', null) == 'group_action') {
            ...
            do_action(BASE_ACTION_AFTER_DELETE, Edutalk_ACL_ROLE, $ids, $result);

            $data['customActionMessage'] = $result ? trans('edutalk-acl::base.delete_role_success') : trans('edutalk-acl::base.delete_role_error');
    ...
    /**
     * @param \Edutalk\Base\ACL\Repositories\PermissionRepository $permissionRepository
     * @return \Illuminate\Contracts\View\Factory|\Illuminate\Http\RedirectResponse|\Illuminate\View\View
     */
    public function getCreate(PermissionRepositoryContract $permissionRepository)
    {
        do_action(BASE_ACTION_BEFORE_CREATE, Edutalk_ACL_ROLE, 'create.get');

        $this->setPageTitle(trans('edutalk-acl::base.create_role'));
        $this->breadcrumbs->addLink(trans('edutalk-acl::base.create_role'));

        $this->dis['permissions'] = $permissionRepository->get();

        return do_filter(BASE_FILTER_CONTROLLER, $this, Edutalk_ACL_ROLE, 'create.get')->viewAdmin('roles.create');
    }

    public function postCreate(CreateRoleRequest $request)
    {
        do_action(BASE_ACTION_BEFORE_CREATE, Edutalk_ACL_ROLE, 'create.post');
        ...
        do_action(BASE_ACTION_AFTER_CREATE, Edutalk_ACL_ROLE, $result);

        $msgType = !$result ? 'danger' : 'success';

        flash_messages()
            ->addMessages(trans('edutalk-acl::base.create_role_' . $msgType), $msgType)
    ...
    public function getEdit(PermissionRepositoryContract $permissionRepository, $id)
    {
        ...
        if (!$item) {
            flash_messages()
                ->addMessages(trans('edutalk-acl::base.role_not_exists'), 'danger')
                ->showMessagesOnSession();

            return redirect()->to(route('admin::acl-roles.index.get'));
        }

        $this->setPageTitle(trans('edutalk-acl::base.edit_role'), '#' . $id . ' ' . $item->name);
        $this->breadcrumbs->addLink(trans('edutalk-acl::base.edit_role'));
        ...
        return do_filter(BASE_FILTER_CONTROLLER, $this, Edutalk_ACL_ROLE, 'edit.get', $id)->viewAdmin('roles.edit');
    }
    ...
    public function postEdit(UpdateRoleRequest $request, $id)
    {
        ...
        $item = do_filter(BASE_FILTER_BEFORE_UPDATE, $item, Edutalk_ACL_ROLE, 'edit.post');
        ...
        do_action(BASE_ACTION_AFTER_UPDATE, Edutalk_ACL_ROLE, $id, $result);

        $msgType = !$result ? 'danger' : 'success';

        flash_messages()
            ->addMessages(trans('edutalk-acl::base.update_role_' . $msgType), $msgType)
            ->showMessagesOnSession();
    ...
    {
        $id = do_filter(BASE_FILTER_BEFORE_DELETE, $id, Edutalk_ACL_ROLE);

        $result = $this->repository->deleteRole($id);

        do_action(BASE_ACTION_AFTER_DELETE, Edutalk_ACL_ROLE, $id, $result);

        $code = $result ? \Constants::SUCCESS_NO_CONTENT_CODE : \Constants::ERROR_CODE;

        $msg = $result ? trans('edutalk-acl::base.delete_role_success') : trans('edutalk-acl::base.delete_role_error');
